 ...
 * ADC2 基础变量
 * 3                 为 磁极感应　　　　　  用到的接口
 * 33,34,35,36,68,69 为 IO口外围模拟量输入  用到的接口
 */
short GV_Adc2Numbers[] = {3};   //【按需设置】ADC2 接口编号集，只有将 ADC2 接口编号加入到这个变量后才能实现对应 ADC2 接口的相关功能


/**
 * ADC2 采样相关变量
 */
short GV_Adc2SamplingAvgCount  = 8;  // ADC2 采样平均值的数量，数量越大平均值越准确
...
float ADC2_GetVoltValue( short number )
{
    ...
    // 返回
    return (float)adc2SamplingTotalValue/GV_Adc2SamplingAvgCount/1024*GV_Adc2SamplingBaseVolt; //电压值 如：2.50V返回为2.50
}
