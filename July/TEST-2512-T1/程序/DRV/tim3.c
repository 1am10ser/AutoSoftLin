...
short GV_TIME3Vlaue = 0;

short GV_TIME33Vlaue = 0;
short GV_TIME34Vlaue = 0;

unsigned char GV_TIME3FLAG = 0;
...
void TIM3_Init()
{
    ...
    TIM3_TimeBaseInit(TIM3_PRESCALER_16, 10);
...
//    if( sendProtectPrev == true && GV_Tim3PwmoSendSoon > 0 ) return false;
//    
//    // 启用上个脉冲发送保护 且 发送个数有限制
//    // 并且 如果未发送完那么该函数不继续执行（相当于此时发送失败）
//    if( sendProtectPrev == true && GV_Tim3PwmoSendLimit > 0 && GV_Tim3PwmoSendLimit >= GV_Tim3PwmoSendCount ) return false;  
//
//    // 启用上个脉冲发送保护 且 发送个数无限制
//    // 并且 如果正在持续发送中那么该函数不继续执行（相当于此时发送失败）
//    if( sendProtectPrev == true && GV_Tim3PwmoSendLimit == 0 && GV_Tim3PwmoSendCount == 1 ) return false; 
//    
//    // 初始发送相关的变量
...
//    //　　5. 滤波频率：经历几个周期相同的跳变则波形稳定。如果频率捕获偶发值不对，那么可以启用 0x02（4个事件）
////    TIM3_PWMIConfig(TIM3_CHANNEL_1, TIM3_ICPOLARITY_RISING, TIM3_ICSELECTION_DIRECTTI, TIM3_ICPSC_DIV1, 0x00);
//    TIM3_ICInit(TIM3_CHANNEL_1, TIM3_ICPOLARITY_RISING, TIM3_ICSELECTION_DIRECTTI, TIM3_ICPSC_DIV1, 0x02);
//
//    
...
//        if(GPIOB->IDR & 0x04)GV_TIME3ZVlaue++;
//        if(GPIOB->IDR & 0x08)GV_TIME3FVlaue++;
        
        if(GV_TIME3Vlaue == 100)
        {
//            TIM3_Cmd(DISABLE);
            GV_TIME3Vlaue = 0;
            GV_TIME3FLAG = 1;    
...
//    static u16 icValue3A, icValue3B, icValue3C;
//
//    static u8 K33 = 0;
//    
//    // 获取定时器输入捕获值
//    if( K33 == 0 )
//    {
//        icValue3A = icValue3B = icValue3C = 0;
//        icValue3A = TIM3_GetCapture1();
//        K33 = 1;
//        
...
//    else
//    {
//        icValue3C = TIM3_GetCapture1() - icValue3A;
//        GV_Tim3PwmiFreq = 1000000/icValue3C;
//        GV_Tim3PwmiDuty = (icValue3B-icValue3A)*100.0/icValue3C;
//        K33 = icValue3A = icValue3B = icValue3C = 0;
//    }
//    
//    // 获取定时器输入捕获值
////    if( icValueA == 0 ){
