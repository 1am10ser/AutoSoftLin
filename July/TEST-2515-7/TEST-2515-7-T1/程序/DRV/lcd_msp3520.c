...
//extern const short GV_LcdChinese16Total;  // 节约单片机 ROM 空间，默认注释掉
...
//extern const struct STRUCT_LCD_CHINESE16 GV_LcdChinese16[];  // 节约单片机 ROM 空间，默认注释掉
...
/**
 * 向 LCD MSP3520 发送颜色数据（正常模式）
 * 基于 LCD_MSP3520_SendByte() 底层发送函数（会等待数据发送成功，速度一般）
 *
 ...
 * @param u16   color       前景色
 * @param u16   background  背景色
 * @param short backfill    背景填充：0表示不填充，正负数表示填充，同时负数会自动绘制边框（其绝对值表示边框宽度）
 */
void LCD_MSP3520_ShowChar( u16 x, u16 y, u8 ascii, u8 size, u16 color, u16 background, short backfill )
{
    // 无效参数
    ...
    // 绘制边框
    if( backfill < 0 ) LCD_MSP3520_Fill(x+backfill, y+backfill, x+size/2-1-backfill, y+size-1-backfill, background);
    ...
    for( i = 0; i < lcdCharBytes; i++ ){
        ...
        //if( size == 32 ) lcdCharByte = GV_LcdAscii1632[ascii][i];  // 调用 16x32 字体  // 节约单片机 ROM 空间，默认注释掉
 ...
 * @param short backfill    背景填充：0表示不填充，正负数表示填充，同时负数会自动绘制边框（其绝对值表示边框宽度）
 */
void LCD_MSP3520_ShowIcon( u16 x, u16 y, u8 index, u8 size, u16 color, u16 background, short backfill )
{
    // 无效参数
    ...
    // 绘制边框
    if( backfill < 0 ) LCD_MSP3520_Fill(x+backfill, y+backfill, x+size-1-backfill, y+size-1-backfill, background);
 ...
 * @param short backfill    背景填充：0表示不填充，正负数表示填充，同时负数会自动绘制边框（其绝对值表示边框宽度）
 */
void LCD_MSP3520_ShowString( u16 x, u16 y, u8 *fonts, u8 size, u16 color, u16 background, short backfill )
{      
    // 无效参数
    if( !(size==12||size==14||size==16||size==24||size==32) ) return;
    ...
    // 绘制边框
    while( *fonts != '\0' ){ fonts++; fontNum++; }
    if( backfill < 0 ) LCD_MSP3520_Fill(x+backfill, y+backfill, x+size/2*fontNum-1-backfill, y+size-1-backfill, background);
 ...
 * @param short backfill    背景填充：0表示不填充，正负数表示填充，同时负数会自动绘制边框（其绝对值表示边框宽度）
 */
void LCD_MSP3520_ShowChinese( u16 x, u16 y, u8 *fonts, u8 size, u16 color, u16 background, short backfill )
{
    // 无效参数
    if( !(size==12||size==14||size==16||size==24||size==32) ) return;
    ...
    // 绘制边框
    while( *fonts != 0 ){ fonts += 2; fontNum++; }
    if( backfill < 0 ) LCD_MSP3520_Fill(x+backfill, y+backfill, x+size*fontNum-1-backfill, y+size-1-backfill, background);
    ...
    // 画汉字
    while( *fonts != 0 ){
        LCD_MSP3520_ShowChineseOne(x, y, fonts, size, color, background, backfill==0?0:1);
        fonts += 2; x += size;
    }
}
 ...
 * @param short backfill    背景填充：0表示不填充，正负数表示填充，同时负数会自动绘制边框（其绝对值表示边框宽度）
 */
void LCD_MSP3520_ShowChineseOne( u16 x, u16 y, u8 *font, u8 size, u16 color, u16 background, short backfill )
{
    // 初始化
    u16 xorigin = x;
    u8 i, k, bit, find;
    ...
    //if( size == 16 ) total = GV_LcdChinese16Total;  // 节约单片机 ROM 空间，默认注释掉
    if( size == 24 ) total = GV_LcdChinese24Total;
    //if( size == 32 ) total = GV_LcdChinese32Total;  // 节约单片机 ROM 空间，默认注释掉
    ...
    for( k = 0; k < total; k++ ){
        ...
        //if( size == 16 && (GV_LcdChinese16[k].Index[0]==*(font)) && (GV_LcdChinese16[k].Index[1]==*(font+1)) ) find = true;  // 节约单片机 ROM 空间，默认注释掉
        ...
        for( i = 0; i < lcdCharBytes; i++ ){
            ...
            for( bit=0; bit<8; bit++ ){    
                ...
                if( backfill != 0 ){
                    ...
                    //if( size == 16 ) LCD_MSP3520_SendColor(GV_LcdChinese16[k].Masks[i]&(0x01<<bit)?color:background);  // 节约单片机 ROM 空间，默认注释掉
                ...
                // 背景忽略
                    ...
                    //if( size == 16 && GV_LcdChinese16[k].Masks[i]&(0x01<<bit) ) LCD_MSP3520_DrawPoint(x, y, color);  // 节约单片机 ROM 空间，默认注释掉
                    if( size == 24 && GV_LcdChinese24[k].Masks[i]&(0x01<<bit) ) LCD_MSP3520_DrawPoint(x, y, color);  
 ...
 */
short LCD_MSP3520_ShowNumber( u16 x, u16 y, double number, u8 size, u16 color, u16 background, short backfill, short showlen )
{
    // 初始化
    ...
    // 显示内容
    if( intnum%100 == 0 ){
        return LCD_MSP3520_ShowInteger(x, y, (long)number, size, color, background, backfill, showlen);
    }else{
        return LCD_MSP3520_ShowFloat(x, y, number, size, color, background, backfill, showlen);
    }
}
void LCD_MSP3520_ShowSplitNumber( u16 x, u16 y, short *splitnums, short splitindex, u8 size, u16 color, u16 background, short backfill, short showlen )
{
    // 初始化
    short i, l;
    short sizex = size/2;
    ...
    // 绘制边框
    short charNum = showlen >= 0 ? showlen : -showlen;
    charNum = charNum > splitindex ? charNum : splitindex;
    if( backfill < 0 ) LCD_MSP3520_Fill(x+backfill, y+backfill, x+(size/2)*charNum-1-backfill, y+size-1-backfill, background);

    // 全部显示
    if( showlen == 0 ){
        for( i = 0; i < splitindex; i++ ){
            if( splitnums[i] >=  0 ) LCD_MSP3520_ShowChar(x+i*sizex, y, splitnums[i]+48, size, color, background, backfill==0?0:1);
            if( splitnums[i] == -1 ) LCD_MSP3520_ShowChar(x+i*sizex, y, '.', size, color, background, backfill==0?0:1);
            if( splitnums[i] == -2 ) LCD_MSP3520_ShowChar(x+i*sizex, y, '-', size, color, background, backfill==0?0:1);
        }
    }
    ...
    if( showlen > 0 ){
        for( l = 0; l < showlen; l++ ){
            i = l;
            if( i < splitindex ){
                if( splitnums[i] >=  0 ) LCD_MSP3520_ShowChar(x+l*sizex, y, splitnums[i]+48, size, color, background, backfill==0?0:1);
                if( splitnums[i] == -1 ) LCD_MSP3520_ShowChar(x+l*sizex, y, '.', size, color, background, backfill==0?0:1);
                if( splitnums[i] == -2 ) LCD_MSP3520_ShowChar(x+l*sizex, y, '-', size, color, background, backfill==0?0:1);
            }else{
                LCD_MSP3520_ShowChar(x+l*sizex, y, ' ', size, color, background, backfill==0?0:1);
            }
        }
    }
    ...
    if( showlen < 0 ){
        ...
        for( l = showlen; l > 0; l-- ){
            ...
            if( i >= 0 && i < splitindex ){
                if( splitnums[i] >= 0  ) LCD_MSP3520_ShowChar(x+(l-1)*sizex, y, splitnums[i]+48, size, color, background, backfill==0?0:1);
                if( splitnums[i] == -1 ) LCD_MSP3520_ShowChar(x+(l-1)*sizex, y, '.', size, color, background, backfill==0?0:1);
                if( splitnums[i] == -2 ) LCD_MSP3520_ShowChar(x+(l-1)*sizex, y, '-', size, color, background, backfill==0?0:1);
            }else{
                LCD_MSP3520_ShowChar(x+(l-1)*sizex, y, ' ', size, color, background, backfill==0?0:1);
            }
        }
    }
}
 ...
 * @param u16   x           起点 X 轴值（横向坐标值）
 * @param u16   y           起点 Y 轴值（纵向坐标值）
 * @param u8    number      十进制淑
 * @param u8    size        字体大小（以 Y 轴值为基准），支持：12, 14, 16, 24, 32
 * @param u16   color       前景色
 * @param u16   background  背景色
 * @param short backfill    背景填充：0表示不填充，正负数表示填充，同时负数会自动绘制边框（其绝对值表示边框宽度）
 * @param bool  showfix     显示前戳
 *
 * @param short 返回实际显示的数字个数（包括0x）
 */
short LCD_MSP3520_ShowHex( u16 x, u16 y, u8 number, u8 size, u16 color, u16 background, short backfill, bool showfix )
{
    // 初始化
    char hexChr[10];
    char hexLib[40] = "0123456789ABCDEF";
    short i, hexNumCnt = 0, hexShowCnt = 0;

    while( number != 0 ){
        hexChr[hexNumCnt++] = hexLib[number%16];
        number = number/16;
    }

    // 显示前戳 0x
    if( showfix == true ){
        LCD_MSP3520_ShowChar((hexShowCnt++)*(size/2)+x, y, '0', size, color, background, backfill);
        LCD_MSP3520_ShowChar((hexShowCnt++)*(size/2)+x, y, 'x', size, color, background, backfill);
    }

    // 补全位数
    if( hexNumCnt == 0 ) hexChr[hexNumCnt++] = '0';
    if( hexNumCnt == 1 ) hexChr[hexNumCnt++] = '0';
    
    // 显示数字
    for( i=hexNumCnt-1; i >= 0; i-- ) LCD_MSP3520_ShowChar((hexShowCnt++)*(size/2)+x, y, hexChr[i], size, color, background, backfill);
 ...
 * @param short 返回实际显示的数字个数（包括小数点和负号）
 */
short LCD_MSP3520_ShowFloat( u16 x, u16 y, double number, u8 size, u16 color, u16 background, short backfill, short showlen )
{
    // 初始化
    short i, onenum;
 ...
 * @param short 返回实际显示的数字个数（包括小数点和负号）
 */
short LCD_MSP3520_ShowInteger( u16 x, u16 y, long number, u8 size, u16 color, u16 background, short backfill, short showlen )
{
    // 初始化
    short i, onenum;
