 ...
 * LIN 调试相关变量
 */
short GV_LinSlaveDebugSendOutUart1      = 0;  // Debug 发送的数据输出到 UART1
short GV_LinSlaveDebugSendOutPrintf     = 0;  // Debug 发送的数据输出到 printf（注意：该系统中可能重载 printf 函数输出到 UART1）
 ...
 * LIN 信号相关变量
 */
u8 GV_LinSlaveSignal[]                     = {0x2E, 0x09, 0x00, 0x00, 0x00};  // 汽车 ON 档时开关默认上发的信号（ID 和 DATA，不含 CheckNum）；所有功能触发都基于该信号构建上发数据
...
u8 GV_LinSlaveReceiveSignal[]              = {0x00, 0x00, 0x00, 0x00};  // 【系统使用】接收信号容器（接收 ECU 下发的数据）
u8 GV_LinSlaveReceiveSignalSurplus         = 0;                         // 【系统使用】接收信号剩余数量
u8 GV_LinSlaveReceiveSignalBLedOpen[]      = {0xD3, 0x44, 0xFF, 0xBB};  // 【按需设置】ID, DATA, CheckNum；ECU 下发的背光灯亮的信号
u8 GV_LinSlaveReceiveSignalWLockLedOpen[]  = {0x14, 0xFD, 0xFF, 0x02};  // 【按需设置】ID, DATA, CheckNum；ECU 下发的车门锁灯亮的信号
...
void LIN_SLAVE_SendSignalResponse()
{
    ...
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalLFU,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalLFUA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseLFUpAuto()   : LIN_SLAVE_SetSignalResponseLFUp();    // 左前升/一键
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalLFD,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalLFDA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseLFDownAuto() : LIN_SLAVE_SetSignalResponseLFDown();  // 左前降/一键
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalLBU,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalLBUA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseLBUpAuto()   : LIN_SLAVE_SetSignalResponseLBUp();    // 左后升/一键
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalLBD,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalLBDA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseLBDownAuto() : LIN_SLAVE_SetSignalResponseLBDown();  // 左后降/一键
    
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalRFU,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalRFUA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseRFUpAuto()   : LIN_SLAVE_SetSignalResponseRFUp();    // 右前升/一键
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalRFD,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalRFDA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseRFDownAuto() : LIN_SLAVE_SetSignalResponseRFDown();  // 右前降/一键
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalRBU,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalRBUA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseRBUpAuto()   : LIN_SLAVE_SetSignalResponseRBUp();    // 右后升/一键
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalRBD,true) == KEY_STATE_YES ) KEY_IsPress(GV_LinSlaveKeyMapSignalRBDA,true) == KEY_STATE_YES ? LIN_SLAVE_SetSignalResponseRBDownAuto() : LIN_SLAVE_SetSignalResponseRBDown();  // 右后降/一键

    if( KEY_IsPress(GV_LinSlaveKeyMapSignalMirrOC ,true) == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseMirrOC();     // 后视镜收放
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalMirrSWR,true) == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseMirrSWR();    // 切换到右后视镜
    
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalMirrUp   ,true) == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseMirrUp();     // 后视镜向上
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalMirrDown ,true) == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseMirrDown();   // 后视镜向下
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalMirrLeft ,true) == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseMirrLeft();   // 后视镜向左
    if( KEY_IsPress(GV_LinSlaveKeyMapSignalMirrRight,true) == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseMirrRight();  // 后视镜向右

    if( KEY_IsPress(GV_LinSlaveKeyMapSignalWinLock,true)   == KEY_STATE_YES ) LIN_SLAVE_SetSignalResponseWinLock();    // 车窗锁
