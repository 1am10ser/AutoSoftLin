...
char MAGNETIC_GetMagnetic( bool sampling )
{
    ...
    // 初始无磁场时电压
    // 注：每颗芯片无磁场时可能不同，下面的 2.40 是采集于当前焊接在板子上芯片（基于测试板供电12V采集的数据）
    float noMagneticVolt = 2.40;  // 无磁场时芯片实际输出电压
    float noMagneticVoltBound = 0.05;  // 无磁场电压值误差（即 noMagneticVolt±noMagneticVoltBound 都认为是磁场）
    
    // 磁极判断
    // 注意：插件和贴片的磁感芯片对于磁铁的南极感应面是不一样的：插件是丝印面，贴片则是背面
...
u16 MAGNETIC_GetMagneticGauss( bool sampling )
{
    ...
    // 每高斯变化对应的电压值
    float voltPerGauss = 0.0015;  // WSH135=1.5mV/Gauss, WSH137=4.4mV/Guass
...
void MAGNETIC_MainMagneticShowLed()
{
    ...
    // 未知磁极 LED 灯闪烁
    if( unknowMagnetic++ > 300 ) unknowMagnetic = 1;
