...
//【按需设置】74HC164 编号集，只有将 74HC164 编号加入到这个变量后才能实现对应 74HC164 的相关功能
//　　　　　　由于 U6,U7,U8,U9,U18 是级联的，所以填写 6 即可启用，无需要填写 7,8,9,18 这三个数字
short GV_HC164Numbers[] = {6};   
...
/**
 * 获取 74HC164 对应芯片上的针脚号（数据脚 DSA,DSB）
 *
...
/**
 * 获取 74HC164 对应芯片上的端口号（数据脚 DSA,DSB）
 *
 * @param short number  PCB 上的 74HC164 编号
...
/**
 * 74HC164 发送数据
 * 该函数为 74HC164 发送字节的程序，优先发送低位
 * 备注：发送一个字节，先输入的数据位最终将在Q7口输出，最后输入的数据位将在Q0口输出
 *
