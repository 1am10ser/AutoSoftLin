...
/**
 * 获取 74HC595 对应芯片上的针脚号（移位时钟脚 SH_CP）
 *
 * @param short number  PCB 上的 74HC595 编号
 * @return GPIO_Pin_TypeDef  无效参数时返回 GPIO_PIN_ALL
 */
...
/**
 * 获取 74HC595 对应芯片上的端口号（移位时钟脚 SH_CP）
 *
 * @param short number  PCB 上的 74HC595 编号
 * @return GPIO_TypeDef*  无效参数时返回 0
 */
...
/**
 * 获取 74HC595 对应芯片上的针脚号（存储时钟脚 ST_CP）
 *
 * @param short number  PCB 上的 74HC595 编号
 * @return GPIO_Pin_TypeDef  无效参数时返回 GPIO_PIN_ALL
 */
...
/**
 * 获取 74HC595 对应芯片上的端口号（存储时钟脚 ST_CP）
 *
 * @param short number  PCB 上的 74HC595 编号
 * @return GPIO_TypeDef*  无效参数时返回 0
 */
 ...
 */
void HC595_SendByteSet( short number, u8 byte )
{
    // 初始化
    u8 i, dat;
...
/**
 * 74HC595 发送数据
 * 由于 74HC595 的锁存机制，所以通过 HC595_SendByteSet() 函数发送数据后并不会马上输出数据到并行口，需要调用该函数进行输出
 */
void HC595_SendByte( short number )
{
    // 初始化
    ...
    // 输出数据
    // 74HC595 第 13（OE）脚为低电平时，然后引脚 12（ST_CP）产生一个上升沿后就会输出
    GPIO_WriteLow(clockPort, clockPin);
    GPIO_WriteHigh(clockPort, clockPin);
}
