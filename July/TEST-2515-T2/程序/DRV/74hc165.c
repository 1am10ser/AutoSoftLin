...
/* KEY TODO...
    ...
    for( int i=0; i < ARR_LENGTH(GV_HC165Numbers); i++ ){
        if( HC165_GetPortExti(GV_HC165Numbers[i]) == 0xFF ) continue;
        EXTI_SetExtIntSensitivity(KEY_GetPortExti(GV_KeyNumbers[i]), EXTI_SENSITIVITY_RISE_FALL);
    }
*/
}
...
/**
 * 获取 74HC165 按键对应芯片上的中断口
 *
 * @param short number  PCB 上的 Key 按键编号
 * @return EXTI_Port_TypeDef  无效参数时返回 0xFF
 */
/* KEY TODO...
EXTI_Port_TypeDef HC165_GetPortExti( short number )
{
    // 适配端口
    if( number == 16 ) return EXTI_PORT_GPIOD;
    
    // 无效返回
    return (EXTI_Port_TypeDef)0xFF;
}
*/
...
GPIO_Pin_TypeDef HC165_GetDataPin( short number )
{
    ...
    if( number == 16 ) return GPIO_PIN_6;
...
/**
 * 读取 74HC165 数据
 * 数据缓存到全局变量 GV_HC165DataValuesU.. 中
 *
 * @param short number  PCB 上的 74HC165 编号
 */
