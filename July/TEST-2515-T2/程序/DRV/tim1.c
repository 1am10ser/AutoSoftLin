...
void TIM1_PWMO_Init( u32 freq )
{
    ...
    for( int i=0; i < ARR_LENGTH(GV_Tim1PwmoNumbers); i++ ){
        ...
        if( GV_Tim1PwmoNumbers[i] == 56 ) TIM1_OC1Init(TIM1_OCMODE_PWM2, TIM1_OUTPUTSTATE_ENABLE, TIM1_OUTPUTNSTATE_ENABLE, 0, TIM1_OCPOLARITY_HIGH, TIM1_OCNPOLARITY_HIGH, TIM1_OCIDLESTATE_RESET, TIM1_OCNIDLESTATE_RESET);
        if( GV_Tim1PwmoNumbers[i] == 57 ) TIM1_OC3Init(TIM1_OCMODE_PWM2, TIM1_OUTPUTSTATE_ENABLE, TIM1_OUTPUTNSTATE_ENABLE, 0, TIM1_OCPOLARITY_HIGH, TIM1_OCNPOLARITY_HIGH, TIM1_OCIDLESTATE_RESET, TIM1_OCNIDLESTATE_RESET);
 ...
 * @param boolean sendProtectPrev  上个脉冲发送保护：上一个脉冲未完全发送前，如果再次调用该函数，那么函数不执行
 * 
...
bool TIM1_PWMO_Send( short number, double duty, u32 sendLimit, short sendProtectPrev )
{
    ...
    if( number == 56 || number == TIM1_NUMBER_ALL ){
        // 启用上个脉冲发送保护 且 有即将发送的脉冲
        // 那么该函数不继续执行（相当于此时发送失败）
        if( sendProtectPrev == true && GV_Tim1PwmoSendSoon56 > 0 ) return false;
        
        // 启用上个脉冲发送保护 且 发送个数有限制
        // 并且 如果未发送完那么该函数不继续执行（相当于此时发送失败）
        if( sendProtectPrev == true && GV_Tim1PwmoSendLimit56 > 0 && GV_Tim1PwmoSendLimit56 >= GV_Tim1PwmoSendCount56 ) return false;  

        // 启用上个脉冲发送保护 且 发送个数无限制
        // 并且 如果正在持续发送中那么该函数不继续执行（相当于此时发送失败）
        if( sendProtectPrev == true && GV_Tim1PwmoSendLimit56 == 0 && GV_Tim1PwmoSendCount56 == 1 ) return false; 
    ...
    if( number == 57 || number == TIM1_NUMBER_ALL ){
        // 启用上个脉冲发送保护 且 有即将发送的脉冲
        // 那么该函数不继续执行（相当于此时发送失败）
        if( sendProtectPrev == true && GV_Tim1PwmoSendSoon57 > 0 ) return false;
        
        // 启用上个脉冲发送保护 且 发送个数有限制
        // 并且 如果未发送完那么该函数不继续执行（相当于此时发送失败）
        if( sendProtectPrev == true && GV_Tim1PwmoSendLimit57 > 0 && GV_Tim1PwmoSendLimit57 >= GV_Tim1PwmoSendCount57 ) return false;  

        // 启用上个脉冲发送保护 且 发送个数无限制
        // 并且 如果正在持续发送中那么该函数不继续执行（相当于此时发送失败）
        if( sendProtectPrev == true && GV_Tim1PwmoSendLimit57 == 0 && GV_Tim1PwmoSendCount57 == 1 ) return false; 
