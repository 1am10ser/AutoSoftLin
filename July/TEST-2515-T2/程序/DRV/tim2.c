 ...
 * 以下变量定义在 a4988.c 文件中
 */
extern bool GV_A4988Motor;
extern bool GV_A4988MotorRuning;
...
bool TIM2_PWMO_Send( double duty, u32 sendLimit, short sendProtectPrev )
{
    ...
    if( sendProtectPrev == true && GV_Tim2PwmoSendSoon > 0 ) return false;
    
    // 启用上个脉冲发送保护 且 发送个数有限制
    // 并且 如果未发送完那么该函数不继续执行（相当于此时发送失败）
    if( sendProtectPrev == true && GV_Tim2PwmoSendLimit > 0 && GV_Tim2PwmoSendLimit >= GV_Tim2PwmoSendCount ) return false;  

    // 启用上个脉冲发送保护 且 发送个数无限制
    // 并且 如果正在持续发送中那么该函数不继续执行（相当于此时发送失败）
    if( sendProtectPrev == true && GV_Tim2PwmoSendLimit == 0 && GV_Tim2PwmoSendCount == 1 ) return false; 
...
void TIM2_PWMO_Stop()
{
    ...
    // 电机运行标记
    if( GV_A4988Motor == true ) GV_A4988MotorRuning = false;
}
...
void TIM2_InterruptHandlerUpdate()
{
    ...
    if( GV_Tim2PwmoSendSoon == 0 && GV_Tim2PwmoSendLimit > 0 ){
        if( GV_Tim2PwmoSendCount++ >= GV_Tim2PwmoSendLimit ) TIM2_PWMO_Stop();  // 如果发送个数超过限制个数，那么暂停输出
    }
    ...
    if( GV_Tim2PwmoSendSoon == 1 ){
        ...
        if( GV_A4988Motor == true ) GV_A4988MotorRuning = true;
    }

    // 电机运行中断
    if( GV_A4988Motor == true && GV_A4988MotorRuning == true ) A4988_InterruptHandlerRuning();
